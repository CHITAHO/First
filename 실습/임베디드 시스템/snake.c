...
void render()
{
	...
	memset(fb, 0, 128);
	fb->pixel[apple.x][apple.y]=0xFFE0;
	fb->pixel[apple2.x][apple2.y]=0xF800;
	fb->pixel[trap.x][trap.y]=0x7FF;
	...
	if (appleCheck) {
		for (seg_i = snake.tail; seg_i; seg_i=seg_i->next) {
			if (seg_i->x == apple.x && seg_i->y == apple.y)
				return 1;
		else if(seg_i->x ==apple2.x && seg_i->y ==apple2.y)
			return 2;
		else if(seg_i->x ==trap.x&& seg_i->y ==trap.y)
			reset();
	}
	return 0;
	}	
...
void game_logic(void)
{
	...
	if (check_collision(1)==1||check_collision(1)==2) {
		...
		snake.tail = new_tail;
			...
			if(check_collision(1)==1){
			apple.x = rand() % 8;
			apple.y = rand() % 8;
			trap.x = rand() % 8;
			trap.y = rand() % 8;
			}
			else if(check_collision(1)==2){
				apple2.x = rand() % 8;
                apple2.y = rand() % 8;
                trap.x = rand() % 8;
                trap.y = rand() % 8;
